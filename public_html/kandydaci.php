<HTML>
    <HEAD>
        ...
        <?PHP
            ...
            $tmp = oci_parse($conn, "SELECT pierwsze_imie, drugie_imie, nazwisko FROM kandydat WHERE id = $kan_id");
            oci_execute($tmp, OCI_NO_AUTO_COMMIT);
            $tmp = oci_fetch_array($tmp, OCI_BOTH);

            $kom = oci_parse($conn, "SELECT nazwa 
                                     FROM kandydat JOIN komitet ON kandydat.id_komitetu = komitet.id 
                                     WHERE kandydat.id = $kan_id");
            oci_execute($kom, OCI_NO_AUTO_COMMIT);
            $kom = oci_fetch_array($kom, OCI_BOTH);
            $kom = $kom[0];
            ...
            if ($pow) {
                $gm_res_all = oci_parse($conn, 
                                        ...
                                        FROM wyniki JOIN lokal ON wyniki.id_lokalu = lokal.id
                                            ...
                                            JOIN powiat ON gmina.id_powiatu = powiat.id
                                            JOIN kandydat ON wyniki.id_kandydata = kandydat.id
                                        WHERE kandydat.id = $kan_id AND powiat.id = $pow
            ...
            if ($gm) {
                ...
                $gm_res_all = oci_parse($conn, 
                                        ...
                                        FROM wyniki JOIN lokal ON wyniki.id_lokalu = lokal.id
                                            ...
                                            JOIN powiat ON gmina.id_powiatu = powiat.id
                                            JOIN kandydat ON wyniki.id_kandydata = kandydat.id
                                        WHERE kandydat.id = $kan_id AND powiat.id = $pow_this
                                        GROUP BY gmina.id, gmina.nazwa
                                        ORDER BY suma DESC");
                oci_execute($gm_res_all, OCI_NO_AUTO_COMMIT);
                ...
                $lok_res_all = oci_parse($conn, 
                                        ...
                                        FROM wyniki JOIN lokal ON wyniki.id_lokalu = lokal.id
                                            JOIN gmina ON lokal.id_gminy = gmina.id
                                            JOIN kandydat ON wyniki.id_kandydata = kandydat.id
                                        WHERE kandydat.id = $kan_id AND gmina.id = $gm
    ...
    <BODY>
        ...
        <H1> Kandydat </H1>
        <?PHP 
            if ($tmp['DRUGIE_IMIE'] == NULL) {
                echo "<H2> $tmp[0] $tmp[2] </H2>";
            } else {
                echo "<H2> $tmp[0] $tmp[1] $tmp[2] </H2>";
            }
        ?>
        <?PHP echo "<H3> $kom </H3>"; ?>
        <div class = "row">
